...
const Sandbox = () => {
  ...
  const setupCanvas = useCallback(() => {
    ...
    for (let i = 0; i < numElements; i++) {
      ...
      const circle = Matter.Bodies.circle(x, y, elementRadius, {
        restitution: 1,
        density: 2,
      });
      circle.label = "circle";
    ...
    const wallLeft = Matter.Bodies.rectangle(
      ...
      wallThickness,
      windowHeight,
      { isStatic: true, fillStyle: "f0f8ff00" },
    );
  ...
  return (
    <div>
      <button id="resetButton" onClick={resetCanvas}>
        Reset
      </button>
